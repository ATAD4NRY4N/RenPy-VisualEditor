#!/usr/bin/env pwsh
# Backend Run Script (Production Mode) - Alternative to Docker
# This script runs the FastAPI backend server in production mode (no auto-reload)

Write-Host "=== RenPy Visual Editor - Starting Backend Server (Production Mode) ===" -ForegroundColor Cyan
Write-Host ""
...
# Set environment variables (similar to docker-compose.yml production)
